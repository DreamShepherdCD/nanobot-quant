...
RUN echo "[bust=684]" && pip install --break-system-packages \
        'mcp<2' \
        git+https://github.com/DreamShepherdCD/nanobot-quant.git@9f81163 \
        git+https://github.com/DreamShepherd2006/Vibe-Trading.git@v0.1.12 \
    && echo "✅ nanobot-quant @9f81163 [CD fork 验证 pin] (C43-① 家族 Δσ 价差模型 + 链页卖方实收(bid) 列 + 回测报告补价差模型行/账目四件套 + 期权回测止盈线接线; bust=684) + vibe-trading @v0.1.12"
